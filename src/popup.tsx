...
function Popup() {
  ...
  useEffect(() => {
    chrome.storage.local.get(
      [THEME_STORAGE_KEY, COLLAPSE_STORAGE_KEY],
      (result) => {
  ...
  const handleCollapseToggle = (enabled: boolean) => {
    setCollapseDevlogs(enabled);
    chrome.storage.local.set({ [COLLAPSE_STORAGE_KEY]: enabled });
  };
  ...
  return (
    <div className="flex flex-col items-center w-[350px] p-6 gap-5 bg-gradient-to-br from-[#1f1f2e] via-[#2b2b3c] to-[#1f1f2e]">
      ...
      <div className="w-full flex flex-col gap-2">
        ...
        <div className="w-full rounded-xl border border-white/[0.08] bg-white/[0.03] divide-y divide-white/[0.06]">
          <label
            htmlFor="toggle-collapse-devlogs"
            className="flex items-center justify-between gap-4 px-4 py-3 cursor-pointer hover:bg-white/[0.04] transition-colors rounded-xl"
          >
